...
class Soal_tkb_project_administration extends CI_Controller
{
	...
	{
		// echo 'A';
		$paket['array'] = $this->db->query("SELECT * FROM tb_soal_tkb_projectadministrationstaff")->result_array();
		$this->load->view('soal/tkb_project_administration/soal', $paket);
	}
	...
	public function tambah()
	{
		...
		if ($this->form_validation->run() == FALSE || $no_soal == 'zero') {
			...
			$this->load->view('soal/tkb_project_administration/vtambah');
		} else {
			$cek_no_soal = $this->db->query("SELECT * FROM tb_soal_tkb_projectadministrationstaff WHERE no_soal=$no_soal")->result_array();
			if (count($cek_no_soal) > 0) {
				...
				$this->db->insert('tb_soal_tkb_projectadministrationstaff', $send);
				$this->session->set_flashdata('msg', 'Data Berhasil Ditambahkan!!!');
				redirect('Soal/Soal_tkb_project_administration/');
			}
		}
	}
	...
	public function edit()
	{
		...
		$this->Mdl_soal->update_data($where, $data, 'tb_soal_tkb_projectadministrationstaff');
	...
	public function hapus($id)
	{
		...
		$this->Mdl_soal->do_delete($where, 'tb_soal_tkb_projectadministrationstaff');
