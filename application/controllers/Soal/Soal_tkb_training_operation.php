...
class Soal_tkb_training_operation extends CI_Controller
{
	...
	{
		// echo 'A';
		$paket['array'] = $this->db->query("SELECT * FROM tb_soal_tkb_trainingoperationstaff")->result_array();
		$this->load->view('soal/tkb_training_operation/soal', $paket);
	}
	...
	public function tambah()
	{
		...
		if ($this->form_validation->run() == FALSE || $no_soal == 'zero') {
			...
			$this->load->view('soal/tkb_training_operation/vtambah');
		} else {
			$cek_no_soal = $this->db->query("SELECT * FROM tb_soal_tkb_trainingoperationstaff WHERE no_soal=$no_soal")->result_array();
			if (count($cek_no_soal) > 0) {
				...
				$this->db->insert('tb_soal_tkb_trainingoperationstaff', $send);
				$this->session->set_flashdata('msg', 'Data Berhasil Ditambahkan!!!');
				redirect('Soal/Soal_tkb_training_operation/');
			}
		}
	}
	...
	public function edit()
	{
		...
		$this->Mdl_soal->update_data($where, $data, 'tb_soal_tkb_trainingoperationstaff');
	...
	public function hapus($id)
	{
		...
		$this->Mdl_soal->do_delete($where, 'tb_soal_tkb_trainingoperationstaff');
