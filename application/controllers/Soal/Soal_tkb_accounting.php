...
class Soal_tkb_accounting extends CI_Controller
{
	...
	{
		// echo 'A';
		$paket['array'] = $this->db->query("SELECT * FROM tb_soal_tkb_accountingstaff")->result_array();
		$this->load->view('soal/tkb_accounting/soal_tkb_accounting', $paket);
	}
	...
	public function tambah()
	{
		...
		if ($this->form_validation->run() == FALSE || $no_soal == 'zero') {
			...
			$this->load->view('soal/tkb_accounting/vtambah_tkb_accounting');
		} else {
			$cek_no_soal = $this->db->query("SELECT * FROM tb_soal_tkb_accountingstaff WHERE no_soal=$no_soal")->result_array();
			if (count($cek_no_soal) > 0) {
				...
				$this->db->insert('tb_soal_tkb_accountingstaff', $send);

				$this->load->view('soal/tkb_accounting/vtambah_tkb_accounting');
				$this->session->set_flashdata('msg', 'Data Berhasil Ditambahkan!!!');
				redirect('Soal/Soal_tkb_accounting/');
			}
		}
	}
	...
	public function edit()
	{
		...
		$this->Mdl_soal->update_data($where, $data, 'tb_soal_tkb_accountingstaff');
	...
	public function hapus($id)
	{
		...
		$this->Mdl_soal->do_delete($where, 'tb_soal_tkb_accountingstaff');
